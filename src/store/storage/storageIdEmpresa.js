...
const ID_EMPRESA_USU = '@id_empresa:key'

export const onSetStorageIdEmpresa = async (idEmpresaBase64) => {
    try {
        await AsyncStorage.setItem(ID_EMPRESA_USU, JSON.stringify(idEmpresaBase64));
        return JSON.stringify(idEmpresaBase64)
    } catch (error) {
        throw (error)
    }
}

export const onGetStorageIdEmpresa = async () => {
    try {
        const user = await AsyncStorage.getItem(ID_EMPRESA_USU);
...
export const onSetStorageSessionidExterno = async (sessionid) => {
    try {
        await AsyncStorage.setItem(SESSIONID_KEY_EXTERNO, JSON.stringify(sessionid));
