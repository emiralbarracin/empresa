...
const TransferenciaDetalle = ({ navigation }) => {
    ...
    const handleComprobante = async () => {
        ...
        try {
            ...
            const { data: res } = await api.get(`api/PDFComprobanteTransferencia/RecuperarPDFComprobanteTransferencia?NombreBanco=${nombreBanco}&Cod_Comprobante=1234567890&Importe_Debitado=${importe}&CBU_CVU_Beneficiario=${cbuDestino}&CUIT_CUIL_CDI_DNI_Beneficiario=${cuil}&Titular_Cta_Destino=${nombre}&Cta_Debito=${cuentaOrigen}&Referencia=VAR&Concepto=OTROS&Plazo_de_Acreditacion=Inmediato&Fecha=${fechaFormateada}&Numero_de_Comprobante=1234567890&Hora=${horaFormateada}&Operacion=Transferencia`);
