...
const PlazoFijoDetalle = ({ navigation }) => {
    ...
    const handleComprobante = async () => {
        ...
        try {
            ...
            const { data: res } = await api.get(`api/PDFComprobantePlazoFijo/RecuperarPDFComprobantePlazoFijo?NombreBanco=${nombreBanco}&Fecha=${fechaFormateada}&Hora=${horaFormateada}&Operacion=Plazo+Fijo&NroCertificado=112640&TipoPlazoFijo=${nombreProducto}&Moneda=${descripcionMoneda}&Capital=${MoneyFormatter(importe)}&Plazo=${plazo}&Tasa=${tna}&Interes=${MoneyFormatter(interes)}&Fechae=${fechaFormateada}&Fechav=${dateFormat(vencimiento)}&CuentaDebito=${codigoCuentaDebito}&CuentaCredito=${codigoCuentaDebito}&Impuesto=0&TotalNeto=${MoneyFormatter(monto)}&RenovacionAutomatica=No`);
